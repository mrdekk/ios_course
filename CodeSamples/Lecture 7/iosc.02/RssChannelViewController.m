...
-( void ) viewWillAppear: ( BOOL )animated
{
	...
	[ [ RssLoader alloc ] initWithURL: [ NSURL URLWithString: @"http://www.itw66.ru/rss/index" ] thenCallTarget: self withSelector: @selector( rssLoaded: ) ];
}
...
-( void ) rssLoaded: ( id )object
{
	if ( [ object isKindOfClass: [ RSS class ] ] )
	{
		[ self setChannel: [ [ ( RSS* )object Channels ] objectAtIndex: 0 ] ];
		[ _tableView reloadData ];
	}
	else if ( [ object isKindOfClass: [ NSError class ] ] )
	{
		UIAlertView* alert = [ [ [ UIAlertView alloc ] initWithTitle: @"Увы!" message: @"Не удалось загрузить RSS" delegate: nil cancelButtonTitle: @"OK" otherButtonTitles: nil ] autorelease ];
		[ alert show ];
	}
}
...
-( UITableViewCell* ) tableView: ( UITableView* )tableView cellForRowAtIndexPath: ( NSIndexPath* )indexPath
{
	...
	else
	{
		...
		cell.Desc.text = [ item.Desc stringByStrippingHTML ];
		if ( cell.Desc.text.length > 100 )
			cell.Desc.text = [ cell.Desc.text substringToIndex: 100 ];
